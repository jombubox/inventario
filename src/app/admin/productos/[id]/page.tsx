...
export default async function ProductDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6">
      ...
      <ProductForm brands={options.brands} componentTypes={options.componentTypes} product={{ id: product.id, sku: product.sku, brandId: product.brandId, componentTypeId: product.componentTypeId, partNumber: product.partNumber, title: product.title, description: product.description, salePrice: product.salePrice, currency: product.currency, status: product.status, isPublic: product.isPublic, updatedAt: product.updatedAt.toISOString(), compatibilities: product.compatibilities.map((item) => ({ brandId: item.brandId, model: item.model, notes: item.notes })) }} />
      <ProductImageManager productId={product.id} canManage images={product.images.map((image) => ({ id: image.id, url: image.url, alt: image.alt, isPrimary: image.isPrimary, sortOrder: image.sortOrder }))} />
