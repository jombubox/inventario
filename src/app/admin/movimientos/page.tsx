...
export default async function MovementsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <div className="mx-auto w-full max-w-[96rem] space-y-6">
    ...
    <Card><CardContent className="p-5"><form method="get" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><Input name="q" defaultValue={query.q} placeholder="Inventario, SKU, producto o motivo" className="lg:col-span-2"/><Select name="type" defaultValue={query.type ?? ""}><option value="">Todos los tipos</option>{inventoryMovementTypeValues.map((type) => <option key={type}>{type}</option>)}</Select><Select name="location" defaultValue={query.location ?? ""}><option value="">Cualquier ubicación</option>{options.locations.map((entry) => <option key={entry.id} value={entry.code}>{entry.code} · {entry.name}</option>)}</Select><Input name="from" type="date" defaultValue={query.from}/><Input name="to" type="date" defaultValue={query.to}/><button className="h-11 rounded-xl bg-navy px-4 text-small font-semibold text-white lg:col-start-6">Aplicar filtros</button></form></CardContent></Card>
    <Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[64rem] text-left text-small"><thead className="border-b border-border bg-muted/60 text-muted-foreground"><tr><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3">Inventario / Producto</th><th className="px-4 py-3">Cantidad</th><th className="px-4 py-3">Ruta</th><th className="px-4 py-3">Motivo</th></tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <tr key={row.id} className="align-top"><td className="whitespace-nowrap px-4 py-3 text-muted-foreground">{formatDateTime(row.createdAt)}</td><td className="px-4 py-3"><Badge variant={movementVariant(row.type)}>{row.type}</Badge></td><td className="px-4 py-3"><Link href={`/admin/inventario?q=${encodeURIComponent(row.inventoryCode)}`} className="font-mono text-xs font-semibold text-primary">{row.inventoryCode}</Link><p className="mt-1 font-semibold text-navy">{row.sku} · {row.productTitle}</p></td><td className="px-4 py-3 font-semibold">{row.quantity}</td><td className="px-4 py-3"><p>{row.fromLocation ?? "Exterior"}</p><p className="text-muted-foreground">→ {row.toLocation ?? "Exterior"}</p></td><td className="max-w-sm px-4 py-3">{row.reason}</td></tr>)}</tbody></table></div>{rows.length === 0 ? <p className="p-8 text-center text-small text-muted-foreground">No hay movimientos para estos filtros.</p> : null}<div className="flex items-center justify-between border-t border-border p-4"><p className="text-small text-muted-foreground">{total} movimiento(s) · página {query.page} de {pageCount}</p><div className="flex gap-2"><Link aria-disabled={query.page <= 1} href={pageHref(Math.max(1, query.page - 1))} className="rounded-lg border border-border px-3 py-2 font-semibold aria-disabled:pointer-events-none aria-disabled:opacity-40">Anterior</Link><Link aria-disabled={query.page >= pageCount} href={pageHref(Math.min(pageCount, query.page + 1))} className="rounded-lg border border-border px-3 py-2 font-semibold aria-disabled:pointer-events-none aria-disabled:opacity-40">Siguiente</Link></div></div></CardContent></Card>
  </div>;
}
